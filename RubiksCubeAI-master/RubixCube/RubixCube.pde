...
void draw() {
  if (!actualPause) {
    ...
    //}
    if (cube.algos.stageNo == 7) {
       println("Solved " + solveCounterThing + " rubik's cubes in " +(cube.algos.globalTimer/1000.0)+ " seconds");
       println("averageTime = " + (float(cube.algos.globalTimer)/float(solveCounterThing)));
    }
